...
{
	// Guoshen Yu, double precision is needed. When the two images are identical, the error under float precision is 0 => log(error)=-inf. 

	int pt_size = (p1.size())/2;

	for (int i = 0; i < pt_size; i++) {
		double x = (double) p1[i*2];
		double y = (double) p1[i*2+1];
		double a = (double) F[1][1]*x+(double) F[1][2]*y+(double) F[1][3];
		double b = (double) F[2][1]*x+(double) F[2][2]*y+(double) F[2][3];
		double c = (double) F[3][1]*x+(double) F[3][2]*y+(double) F[3][3];
		double d = (a*(double) p2[i*2]+b*(double) p2[i*2+1]+c);
		e[i*2] = (d*d)/(a*a+b*b); 
...
void divide_match(const std::vector<Match>& matches, std::vector<float>& p1, std::vector<float>& p2)
{
  ...
  for(; it != matches.end(); ++it) {
    float x1 = (*it).x1; float y1 = (*it).y1;
    float x2 = (*it).x2; float y2 = (*it).y2;
